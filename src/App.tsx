...
import type { CreditEntryRecord, PageId, RechargeAdjustmentLedger, SpecialRecord } from "./types";
...
export default function App() {
  ...
  const routeAdjustment = (ledger: RechargeAdjustmentLedger) => {
    if (ledger.direction === "增加") {
      setGeneratedEntries((current) => [{
        ...
        businessNature: ledger.businessNature as CreditEntryRecord["businessNature"],
        actualAmount: "/",
        giftAmount: "/",
        paymentMethod: "/",
        paymentReference: "/",
    ...
    setGeneratedSpecials((current) => [{
      ...
      month: ledger.occurredAt.slice(0, 7),
      type: "后台手动扣减",
      businessNature: ledger.businessNature as SpecialRecord["businessNature"],
      occurredAt: ledger.occurredAt,
      planPoints: 0,
      rechargePoints: ledger.delta,
      giftPoints: 0,
  ...
  return (
    ...
    >
      {activePage === "credits"
        ? <CreditAccountsPage accounts={accountRecords} onAccountsChange={setAccountRecords} onAdjustment={routeAdjustment} />
        : <FinanceDataPage accountRecords={accountRecords} additionalEntries={generatedEntries} additionalSpecials={generatedSpecials} onOpenCreditAccounts={() => setActivePage("credits")} />}
    </AdminShell>
  );
}
