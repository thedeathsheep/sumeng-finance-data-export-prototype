...
const headers: Record<TabId, string[]> = {
  ...
  monthly: ["会计月份", "当月新增充值总本金", "当月赠送总金额", "当月赊销未回款充值总额", "当月本金消费总额", "当月赠送额度消耗总额", "当月支付手续费", "口径说明"],
  special: ["记录编号", "业务类型", "财务主体", "客户类型", "发生月份", "支付渠道", "关联账户", "原充值订单 / 原积分流水", "资金状态", "积分调整流水", "本金 / 手续费金额", "赠送金额", "套餐积分变动", "充值积分变动", "赠送积分变动", "原因 / 备注", "凭证 / 证明材料", "来源", "操作人", "更新时间", "业务状态", "操作"],
};
...
function cellsFor(tab: TabId, row: FinanceRow, handlers: Pick<FinanceTableProps, "onEdit" | "onVoid" | "onHistory">) {
  ...
  if (tab === "monthly") {
    ...
    return [item.month, money(item.newRechargePrincipal), valueCell(item.giftAmount), money(item.uncollectedRechargeAmount), valueCell(item.principalConsumptionAmount), valueCell(item.giftConsumptionAmount), valueCell(item.paymentFeeAmount), item.note];
  }
  const item = row as SpecialRecord;
  return [item.id, <span className="special-type">{item.type}</span>, valueCell(item.financeProfileName), valueCell(item.customerType ?? "/"), item.month, valueCell(item.paymentChannel ?? "/"), valueCell(item.accountName), valueCell(item.relatedRecordId), <StatusBadge value={item.fundStatus} />, valueCell(item.pointsLedgerId), valueCell(item.principalAmount), valueCell(item.giftAmount), pointCell(item.planPoints), pointCell(item.rechargePoints), pointCell(item.giftPoints), item.reason, valueCell(item.attachment), <StatusBadge value={item.source} />, item.operator, item.updatedAt, <StatusBadge value={item.status} />, actionCell(tab, item.id, item.source, item.status, handlers)];
}
