...
export function ExportDialog({ open, month, accountType, accountName, activeTab, rowCount, totalRowCount, onClose }: ExportDialogProps) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
    <section className="export-dialog" role="dialog" aria-modal="true" aria-labelledby="export-title">
      <header><span className="dialog-icon"><FileSpreadsheet size={20} /></span><div><h2 id="export-title">导出财务数据</h2><p>按当前筛选条件生成与财务表格对应的工作表</p></div><button ref={closeRef} className="icon-button" aria-label="关闭导出弹窗" onClick={onClose}><X size={18} /></button></header>
      {state === "confirm" && <>
        ...
        <aside className="dialog-note">工作表字段按财务原始清单覆盖。系统同步和人工录入记录均标明来源；暂无数据统一导出“/”；已作废记录保留状态但不参与月度汇总。</aside>
      </>}
