...
const tabScopeNotes: Record<TabId, string> = {
  ...
  consumptions:
    "消费只同步真实积分流水，三类积分分别展示；没有可靠人民币换算依据时，金额字段显示“/”。",
  monthly:
    "月度汇总只使用可靠人民币事实；存在应计入但金额缺失的记录时，对应指标显示“/”并说明缺失数量。",
  special:
    "后台充值积分扣减自动生成特殊业务；退款由财务补充线下对公转账资料，原积分事实不可修改。",
};
    ...
    () =>
      consumptions.filter(
        (item) =>
          ...
          statusMatches("有效") &&
          (!normalizedBusiness ||
            [item.scene, item.service, item.relatedType]
              .join(" ")
              .toLocaleLowerCase("zh-CN")
              .includes(normalizedBusiness)),
      ),
  ...
  return (
    <>
      ...
      <section className="scope-notice dependency-notice">
        ...
        <p>
          <strong>自动闭环：</strong>
          后台增加充值积分后自动进入充值明细，扣减后自动进入特殊业务；财务看板不提供人工入账入口。
        </p>
